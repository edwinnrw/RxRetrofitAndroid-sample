...
class PagingRequestHelper(val retryService: Executor){
    internal val mListeners = CopyOnWriteArrayList<Listener>()
    private val mLock = Any()
    private lateinit var mRetryService: Executor
    @GuardedBy("mLock")
    ...
    private fun prepareStatusReportLocked(): StatusReport {
        val errors =
            mRequestQueues[0].mLastError?.let { mRequestQueues[1].mLastError?.let { it1 ->
                mRequestQueues[2].mLastError?.let { it2 ->
                    arrayOf<Throwable>(it,
                        it1, it2
                    )
                }
            } }
    ...
    class StatusReport internal constructor(
        ...
        fun getErrorFor(type: RequestType): Throwable? {
            return mErrors?.get(type.ordinal)
        }
